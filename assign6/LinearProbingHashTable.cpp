...
LinearProbingHashTable::LinearProbingHashTable(HashFunction<string> hashFn) {
    // Store the Hash Function as a member
    myhashFn = hashFn;

    // Number of Slots created by the Hash Function
    numOfSlot = myhashFn.numSlots();

    // Number of Elements stored
    numOfElem = 0;

    // Create the new array of struct Slot
    elems = new Slot[numOfSlot];

    // Innitialize the slot status of the array
...
bool LinearProbingHashTable::insert(const string& elem) {
    ...
    // The table is full or Duplicate element
    if (numOfElem == numOfSlot || contains(elem)) {
        return false;
    }

    // Calculate the hash index of the new element
    int index = myhashFn(elem);


    // There must be valid slot since the table is not full
    while (elems[index].type == SlotType::FILLED) {
        index = (index + 1) % numOfSlot;
    }

    // Insert the element into NOT 'FILLED' slot
    numOfElem++;
...
bool LinearProbingHashTable::contains(const string& elem) const {
    // Return whether the array contains certain element
    // The running time should be O(1)
    ...
    // The hash code (starting code)
    int index = myhashFn(elem);

    // The number of steps - to step over the iteration
    int cnt = 0;

    // Stop: (1) reach empty slot (not full), no more filled slot afterwards; (2) iter over all slots
    while (elems[index].type != SlotType::EMPTY && cnt++ <= numOfSlot) {
...
bool LinearProbingHashTable::remove(const string& elem) {
    // Remove element from the table by changing the status
    // Empty array
    if (!numOfElem) {
        return false;
    }
