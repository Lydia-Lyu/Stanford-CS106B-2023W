...
ParticleSystem::ParticleSystem() {
    // head of the system pointing to the ParicleCell: nullptr
    head = nullptr;
    // Initialize the dynamic `pointer`
    cur = head;
}

ParticleSystem::~ParticleSystem() {
    // If the system is empty
    if (numofParticle == 0 && cur == nullptr && head == nullptr) {
        return;
    }

    while (head) {
        ParticleCell* tmp = head;
        head = head->next;
        delete tmp;
    }
...
int ParticleSystem::numParticles() const {
    // Return private member variable
    return numofParticle;
}

void ParticleSystem::add(const Particle& data) {
    // Invalid Pariticle
    if (data.lifetime < 0 || data.x < 0 || data.y < 0 || data.x >= SCENE_WIDTH || data.y >= SCENE_HEIGHT) {
        return;
    }

    ParticleCell* tmp = new ParticleCell;
    ...
    // Non-empty system
        // `cur` always points to the end of the linked list
        tmp->prev = cur;
        cur->next = tmp;
    ...
    // Add a new cell count
    numofParticle++;
}

void ParticleSystem::drawParticles() const {
    // Draw all particles in the system by the order
    // Empty system
    if (!numofParticle){
        return;
    }

    // Initialize another new pointer
    ParticleCell* draw = head;
    while (draw) {
        if (draw->particle.x >= 0 && draw->particle.x <= SCENE_WIDTH && draw->particle.y >= 0 && draw->particle.y <= SCENE_HEIGHT) {
...
void ParticleSystem::moveParticles() {
    ...
    while (move) {
        ...
        if (move->particle.type == ParticleType::BALLISTIC || move->particle.type == ParticleType::FIREWORK) {
            move->particle.dy++;
        }

        // Remove the particle
        if (move->particle.lifetime < 0 || move->particle.x < 0 || move->particle.y < 0 || move->particle.x > SCENE_WIDTH || move->particle.y > SCENE_HEIGHT) {
            ParticleCell* remove = move;
            if (!remove) {
                continue;
            }
            // Single node link
            ...
            // If a FIREWORK particle explods
            if (remove->particle.type == ParticleType::FIREWORK && remove->particle.lifetime < 0) {
                Color c = Color::random();
                // Generate 50 new particles replacing the exploded FIREWORK particle
                for (int i = 0; i < 50; i++) {
                    // `data` points to (stores) the address of the new Particle object
                    Particle* data = new Particle;
                    data->x = remove->particle.x;
                    data->y = remove->particle.y;
                    data->dx = randomReal(-3, 3);
                    data->dy = randomReal(-3, 3);
                    data->lifetime = randomInteger(2, 10);
                    data->color = c;
                    data->type = ParticleType::STREAMER;

                    // Add the new particle into the system
                    // The `add` function requires parameter of the address of the Particle to be added
                    // The address of the new Particle is stored in
                    add(*data);
                }
            }
            ...
            // Not need to remove the particle
            move = move->next;
        }
    }
}
