...
void freeDoublyLinkedList(Cell* list) {
    // Free memory -> destruct the pointers
    while (list) {
        // Temporarily store the cell
        Cell* tmp = list;
        // Iter the pointer to the next cell: or delete current cell would make next cells unreachable
        list = list->next;
        // Free memory of current cell
        delete tmp;
    }
}
...
void prependTo(Cell*& list, const string& value) {
    // Insert the new value to the front
    Cell* fst = new Cell;
    fst->value = value;
